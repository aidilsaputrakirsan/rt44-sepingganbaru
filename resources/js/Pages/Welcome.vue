...
import { LogIn, Shield, CalendarDays, Receipt, Eye, IdCard, FileSignature, Download, FileText } from 'lucide-vue-next';
...
const availableSurat = computed(() => props.suratPernyataan.filter(s => s.available));
...
<template>
    ...
    <div class="min-h-screen flex flex-col bg-gradient-to-br from-amber-50/60 via-white to-orange-50/40">
        ...
        <!-- Surat Pernyataan (download publik) -->
        <section v-if="availableSurat.length" id="surat-pernyataan" class="relative z-10 w-full px-6 py-14 sm:py-16 bg-white/70 backdrop-blur border-t border-amber-100">
            <div class="max-w-5xl mx-auto">
                <div class="text-center max-w-2xl mx-auto space-y-3 mb-8 sm:mb-10">
                    <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold tracking-wide uppercase">
                        <FileSignature class="w-3.5 h-3.5" />
                        Layanan Warga
                    </div>
                    <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-800">
                        Unduh Blangko <span class="text-amber-500">Surat Pernyataan</span>
                    </h2>
                    <p class="text-sm sm:text-base text-slate-500 leading-relaxed">
                        Unduh blangko kosong (format Word), isi datanya, lalu bubuhkan materai 10.000 dan tanda tangan
                        untuk diketahui Ketua RT. Tersedia gratis untuk seluruh warga RT-44.
                    </p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div
                        v-for="item in availableSurat"
                        :key="item.slug"
                        class="group flex flex-col rounded-xl border border-amber-100 bg-white p-5 shadow-sm transition hover:shadow-md hover:border-amber-200"
                    >
                        <div class="flex items-start gap-3">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-600 group-hover:bg-amber-500 group-hover:text-white transition">
                                <FileText class="h-5 w-5" />
                            </div>
                            <h3 class="font-semibold text-slate-800 leading-snug">{{ item.judul }}</h3>
                        </div>
                        <p class="mt-3 flex-1 text-sm leading-relaxed text-slate-500">{{ item.deskripsi }}</p>
                        <a
                            :href="route('surat-pernyataan.public-download', item.id)"
                            class="mt-4 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold bg-amber-500 text-white hover:bg-amber-600 transition shadow-sm shadow-amber-500/20"
                        >
